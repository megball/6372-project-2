...
#TBD on how to deal with default = uknown
...
#Check for correlation with categorical variables

We cannot keep both age group and age. Should we remove age?

We also need to either remove duration or duration group. 

```{r}
#run two models to see which variable is better?
...
RFresults <- rfe(downsample[,1:21], downsample[[22]], sizes=subsets, rfeControl=RFcontrol)
RFresults
varImp(RFresults)
```
...
# Random Forest
```{r}
fitRF <- train(y ~ duration + duration_group + euribor3m + nr_employed + month, 
               data = train, 
               method = "rf", 

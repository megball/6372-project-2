...
full.numeric <- full[ , sapply(full, is.numeric)]

corrs = cor(full.numeric, use="everything") # Calculate correlations between all variables
high_corrs = findCorrelation(corrs, cutoff=0.1)  # Find 'high' correlations among those variables (0.1 is not exactly "high" but there are so few numerics...)
corrs = cor(full.numeric[,high_corrs], use="everything") # get a data frame with only highly correlated variables

#Create corrplot for numeric variables
corrplot(corrs)

# pairs only on highly correlated variables... 
pairs(corrs,col=full$y)
...
#Random Forest
```{r}
fitRF <- train(y ~ duration + duration_group + euribor3m + nr_employed + month + emp_var_rate, 
               data = train, 
               method = "rf", 
...
I ran a simple Random Forest on the top 6 variables from Megan's recursive feature elimination above. I got an accuracy over 90% on the training data. Let's see how we do on the test data. 
...
Over 91% accuracy on the test data! Also, sensitivity is over 96% while specificity is not that great at 53.79%

...
Still have very high VIF for emp_var_rate, euribor3m, and nr_employed. Age and Age_Grp are also correlated, so remove Age_grp due to higher VIF. Remove those and re-run.



```{r}
train_simple_2 <- train_simple %>% dplyr::select(-emp_var_rate, -euribor3m, -nr_employed, -Age_Grp)
...
```{r}
train_simple_3 <- train_simple_2 %>% dplyr::select(-age, -housing, -loan, -day_of_week)
...
```{r}
complex.log<-glm(y~ education + default + month + duration + campaign + poutcome + cons_price_idx + euribor3m + Age_Grp + Age_Grp*education + campaign*duration + default*poutcome + cons_price_idx*euribor3m + month* euribor3m,family="binomial",data=train)
summary(complex.log)
#exp(cbind("Odds ratio" = coef(complex.log), confint.default(complex.log, level = 0.95)))
...
Duration and campaign is significant but has a small coefficient/odds ratio. Month and euribor3m are significant as an interaction term with some odds ratios being very large. Education and age group is not significant for interaction. 
...
Add in some interactions based on what we see from these charts: education/empvarrate, default/duration, default/euribor3m, default/month and keeping in month/euribor3m

```{r}
complex.log<-glm(y~ education + default + month + duration + campaign + poutcome + cons_price_idx + euribor3m + Age_Grp + month* euribor3m + education * emp_var_rate + default * duration + default * euribor3m + default * month,family="binomial",data=train)
summary(complex.log)

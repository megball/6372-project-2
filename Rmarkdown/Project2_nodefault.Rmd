---
title: "Project2 - removing default"
...
summary(df)
```
Default only has 3 in the yes group. This will likely impact our model performance, especially if we do not have enough "yes" in train/test. Since there is not much value in default as "no" versus default as "unknown", let's just remove the variable completely.

We also have a small illiterate group with only 18 observations. Let's remove this group and narrow our scope as we cannot adequately group this in with another education level.

```{r}
df <- df %>%  filter(education != "illiterate")
nrow(df)
#down to 38,227

#re-factor education
df$education <- droplevels.factor(df$education)

df <- df %>%  dplyr::select(-c("default"))
summary(df)
#38227 obs of 20 vars
...
```{r}
ggplot(df, aes(x=age, y=cons_price_idx, fill = y)) +  geom_point(size=1, shape="circle") +   ggtitle("Consumer Price Index vs Age")
...
ggplot(df) +   
  ...
  ggtitle("Number of 'y' by job") +  ylab("Count") +   
  xlab("Job")
```
...
df$duration_group <-
  cut(
    ...
    labels = c("0-5min", "5-10min", "10+ min")
  )
# Check for missing values
tibble(variable = names(colSums(is.na(df))),
  ...
  tab_header(title = "Missing Values in Data")
df3 <-
  df %>%  group_by(duration_group) %>%  count(y) %>%  mutate(duration_group_conv = n /
                                                               sum(n)) %>%  filter(y == "yes")
...
set.seed(1)
downsample <- downSample(x = sample10k[, -23],
y = sample10k$y)
table(downsample$Class)
RFcontrol <- rfeControl(functions=rfFuncs, method="cv", number=5, verbose = FALSE)
set.seed(123)
subsets <- c(1:5, 10, 15, 20)
RFresults <- rfe(downsample[,1:22], downsample[[23]], sizes=subsets, rfeControl=RFcontrol)
RFresults
varImp(RFresults)
```
Top 12 variables according to ranodm forest: duration, duration_group, month, euribor3m, nr_employed, emp_var_rate, cons_conf_idx, cons_price_idx, pdays, poutcome, contact, Age_Grp

```{r}
#save dataset to this point
#df_nodefault <- write.csv(df, "df_nodefault.csv", row.names = FALSE)

#open saved dataframe
#df <- read.csv("df_nodefault.csv"), stringsAsFactors = TRUE)
#str(df)
```
...
nrow(train %>% filter(y=='yes')) #3,403
nrow(test %>% filter(y=='yes'))  #851

summary(train)
#30581 obs. of 23 variables

#train_nodefault <- write.csv(train, "train_nodefault.csv", row.names = FALSE)
#test_nodefault <- write.csv(test, "test_nodefault.csv", row.names = FALSE)
```
...
train_simple <- train %>% dplyr::select(-pdays,-prevly_Cntctd )
...
Still have very high VIF for emp_var_rate, euribor3m, and nr_employed. Remove those and re-run.



```{r}
train_simple_2 <- train_simple %>% dplyr::select(-emp_var_rate, -euribor3m, -nr_employed)
...
Remove Age_grp. suspect they are flagging due to correlation. 

```{r}
train_simple_3 <- train_simple_2 %>% dplyr::select(-Age_Grp)
...
```{r}
train_simple_4 <- train_simple_3 %>% dplyr::select(-age, -housing, -loan, -day_of_week)
...
#simple model -1 
simple.log<-glm(y~job+marital+education+contact+month+duration+campaign+previous+poutcome+cons_price_idx+cons_conf_idx+duration_group,family="binomial",data=train)
#simple.log<-glm(y~.,family="binomial",data=train_simple_3)
summary(simple.log)
exp(cbind("Odds ratio" = coef(simple.log), confint.default(simple.log, level = 0.95)))
vif(simple.log)
...
Remove pdays as the previously contacted variables is a better demonstration of this item. day_of_week also no longer significant

```{r}
train_step_2 <- train_step %>% dplyr::select(-pdays, -day_of_week)
...
euribor3m and nr_employed have high VIF but both are significant - keep euribor3m as its coefficient is larger. also remove duration group as it correlates to duration and want to avoid multicollinearity

```{r}
train_step_3 <- train_step_2 %>% dplyr::select(-nr_employed, -duration_group)
#Check vifs again
...
vif(step.log)
```
poutcome has high vif still, remove and re-run

```{r}
train_step_4 <- train_step_3 %>% dplyr::select(-poutcome)
#Check vifs again
full.log<-glm(y~.,family="binomial",data=train_step_4)
step.log<-full.log %>% stepAIC(trace=FALSE)
summary(step.log)
#exp(cbind("Odds ratio" = coef(step.log), confint.default(step.log, level = 0.95)))
vif(step.log)
...
full.log<-glm(y~education+contact+month+duration+campaign+previous+cons_price_idx+cons_conf_idx+euribor3m+Age_Grp+prevly_Cntctd,family="binomial",data=train)
#full.log<-glm(y~.,family="binomial",data=train)
step.log<-full.log %>% stepAIC(trace=FALSE)
summary(step.log)
```
...
MB: kept getting same error as Rinku and couldn't run this part :(

```{r}
#dat.train.x <- model.matrix(y~.,train)
#dat.train.x <- as.matrix(dat.train.x)
#dat.train.y<-train[,23]
...
Sensitivity <- cbind(Sensitivity_simple,Sensitivity_step$Step_Sensitivity)
Specificity <- cbind(Specificity_simple, Specificity_step$Step_Specificity)
Accuracy <- cbind(Accuracy_simple,Accuracy_step$Step_Accuracy)
Sensitivity
Specificity
Accuracy
```
...
complex.log<-glm(y~ education + contact + month + duration + campaign + previous + cons_price_idx + cons_conf_idx + euribor3m + Age_Grp + prevly_Cntctd + duration*contact,family="binomial",data=train)
summary(complex.log)
...
```{r}
complex.log<-glm(y~ education + contact + month + duration + campaign + previous + cons_price_idx + cons_conf_idx + euribor3m + Age_Grp + prevly_Cntctd + duration*contact + Age_Grp*education + campaign*duration + cons_price_idx*euribor3m + month*euribor3m,family="binomial",data=train)
summary(complex.log)
#exp(cbind("Odds ratio" = coef(complex.log), confint.default(complex.log, level = 0.95)))
...
Duration and campaign is significant but has a small coefficient/odds ratio. Month and euribor3m are significant as an interaction term but month alone isn't very significant. Education and age group is not significant for interaction. 
...
```{r}
#numerical y vars
ggplot(df, aes(x=education , y=emp_var_rate, fill = y)) +  geom_boxplot() +   ggtitle("Education vs. emp.var.rate")
...
prop.table(table(df_yes$contact,df_yes$prevly_Cntctd),2)
prop.table(table(df_No$contact,df_No$prevly_Cntctd),2)
```

Add in some interactions based on what we see from these charts: contact/duration, contact/previouslycontacted and keeping in duration/campaign

```{r}
complex.log<-glm(y~ education + contact + month + duration + campaign + previous + cons_price_idx + cons_conf_idx + euribor3m + Age_Grp + prevly_Cntctd + contact*duration + contact*prevly_Cntctd + duration*campaign, family="binomial",data=train)
summary(complex.log)
...
#Check metrics with standard 0.5 accuracy and our 0.15 accuracy for simple log model
...
# 0.9110  0.9185  0.9205  0.9210  0.9240  0.9308  
```
...
fit.lda_step <- lda(train.lda.y ~ duration + campaign + cons_price_idx + cons_conf_idx + euribor3m, data = train.lda.x)
...
summary(cv.aucs_2)
  # Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 #0.8987  0.9041  0.9065  0.9071  0.9096  0.9175 
```
...
summary(cv.aucs_qda)
#  Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 0.8864  0.8915  0.8956  0.8965  0.9013  0.9084
```

### QDA on train set
```{r}
fit.qda <- qda(train.lda.y ~ duration + campaign + cons_price_idx + cons_conf_idx + euribor3m, data = train.lda.x)
...
for (i in 1:nloops){
  ...
  cvfit_shuf <- lda(cvtrain.yshuf ~ duration + campaign + cons_price_idx + cons_conf_idx + euribor3m, data = cvtrain.x)
...
summary(cv.aucs_shuf)
 ##  Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
 #0.4698  0.4912  0.4988  0.4988  0.5051  0.5312 
```
...
confusionMatrix(fitRF.predictions.raw, test$y)
```

Using the default cutoff, our random forest gets a test accuracy of 0.9145, with Sensitivity 0.9663 and Specificity 0.5006.
